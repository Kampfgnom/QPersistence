...
{
    QSharedPointer<ParentObject> parent = parentObjectOneToOne();
    if(parent) {
...
{
    QSharedPointer<ParentObject> parent = parentObjectOneToMany();
    if(parent) {
        parent->removeChildObjectOneToMany(Qp::sharedFrom(this));
    }
