...
template<class T, class ... Superclasses> void registerClass() {
    return QpStorage::defaultStorage()->registerClass<T, Superclasses ...>();
}
template<class T> QpDao<T> *dataAccessObject() {
    return QpStorage::defaultStorage()->dataAccessObject<T>();
}
template<class T> QSharedPointer<T> read(int id) {
    return QpStorage::defaultStorage()->read<T>(id);
}
template<class T> QList<QSharedPointer<T> > readAll(const QpSqlCondition &condition = QpSqlCondition()) {
    return QpStorage::defaultStorage()->readAll<T>(condition);
}
template<class T> int count(const QpSqlCondition &condition = QpSqlCondition()) {
    return QpStorage::defaultStorage()->count<T>(condition);
}
template<class T> QSharedPointer<T> create() {
    return QpStorage::defaultStorage()->create<T>();
}
template<class T> UpdateResult update(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->update(object);
}
template<class T> bool remove(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->remove(object);
}
template<class T> bool markAsDeleted(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->markAsDeleted(object);
}
template<class T> bool undelete(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->undelete(object);
}
template<class T> bool isDeleted(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->isDeleted(object);
}
template<class T> SynchronizeResult synchronize(QSharedPointer<T> object, QpDaoBase::SynchronizeMode mode = QpDaoBase::NormalMode) {
    return QpStorage::defaultStorage()->synchronize(object, mode);
}
template<class T> bool incrementNumericColumn(QSharedPointer<T> object, const QString &fieldName) {
    return QpStorage::defaultStorage()->incrementNumericColumn(object, fieldName);
}
#ifndef QP_NO_TIMESTAMPS
template<class T> QList<QSharedPointer<T> > createdSince(const QDateTime &time) {
    return QpStorage::defaultStorage()->createdSince<T>(time);
}
template<class T> QList<QSharedPointer<T> > updatedSince(const QDateTime &time) {
    return QpStorage::defaultStorage()->updatedSince<T>(time);
}
template<class T> QDateTime creationTime(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->creationTime(object);
}
template<class T> QDateTime creationTimeInDatabase(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->creationTimeInDatabase(object);
}
template<class T> QDateTime creationTimeInInObject(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->creationTimeInInObject(object);
}
template<class T> QDateTime updateTimeInDatabase(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->updateTimeInDatabase(object);
}
template<class T> QDateTime updateTimeInObject(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->updateTimeInObject(object);
}
#endif
#ifndef QP_NO_LOCKS
...
template<class T> QpLock unlock(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->unlock(object);
}
template<class T> QpLock lockStatus(QSharedPointer<T> object) {
    return QpStorage::defaultStorage()->lockStatus(object);
}
#endif
